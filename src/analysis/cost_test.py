...
class BenchmarkRunner:
    def __init__(self, model, cfg, detector_type, window_samples):
        ...
        # 🚨 唯一的最小修复：防止 KSWIN 随机采样越界报错
        if detector_type == "kswin":
            self.det_config['stat_size'] = min(1000, window_samples // 3)
    ...
    def run_stream(self, dataloader):
        ...
        for batch in progress:
            ...
            if torch.cuda.is_available(): torch.cuda.synchronize()  # 确保 GPU 计算完成
