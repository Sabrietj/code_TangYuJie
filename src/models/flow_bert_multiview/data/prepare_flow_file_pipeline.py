...
def prepare_sampled_data_files(cfg):
    ...
    if cfg.data.sampling.enabled and cfg.data.sampling.sample_ratio < 1.0:
        if cfg.data.split_mode == "flow":
            ...
            # 根据配置决定文件名后缀
            is_random = getattr(cfg.data.sampling, "random", True)  # 默认为 True (随机)
            logger.info(f"是否随机下采样: {is_random}")
            method_tag = "sampled" if is_random else "ordered_sampled"

            sampled_flow_data_path = flow_data_path.replace(
                ".csv",
                f".{method_tag}_{cfg.data.sampling.sample_ratio}_{cfg.data.random_state}.csv"
            )
            ...
            if not os.path.exists(sampled_flow_data_path):
                if is_rank0():
                    if is_random:
                        ...
                        # 新增的有序采样
                        logger.info("执行有序流采样 (Ordered Sampling)...")
                        sample_flow_csv_ordered(
                            input_path=flow_data_path,
                            output_path=sampled_flow_data_path,
                            ratio=cfg.data.sampling.sample_ratio,
                        )
            ...
            logger.info(f"使用下采样后的网络流数据文件: {sampled_flow_data_path}")
...
def sample_flow_csv_ordered(
    ...
    """
    保留时序的采样函数。
    假设 input_path 中的数据已经按时间排序。
    使用间隔采样 (Systematic Sampling) 来减少数据量，同时保持时间覆盖范围和顺序。
    """
    first = True
    total_seen = 0
    total_kept = 0

    # 计算步长：例如 ratio=0.1 (10%)，则 1/0.1 = 10，即每 10 条取 1 条
    step = int(1 / ratio) if ratio > 0 else 1
    step = max(1, step)

    logger.info(f"应用有序采样 (Ordered Sampling): ratio={ratio}, step={step}")

    # 使用 iterator 分块读取，避免内存溢出
    reader = pd.read_csv(input_path, chunksize=chunksize, low_memory=False)
    if is_rank0():
        reader = tqdm(reader, desc="Ordered Sampling flows", unit="chunk")

    # 维护一个全局索引，确保跨 chunk 的步长连续性
    global_idx = 0
    ...
    for chunk in reader:
        ...
        # 计算当前 chunk 的起始偏移量
        # 逻辑：(global_idx + local_i) % step == 0
        # start_offset 保证了即使跨越了 chunk 边界，采样的间隔依然是均匀的
        start_offset = (step - (global_idx % step)) % step

        # 使用切片操作进行采样，切片天然保持顺序
        sampled = chunk.iloc[start_offset::step]
    ...
    logger.info(
        f"[sample_flow_csv_ordered] kept {total_kept} / {total_seen} flows "
        f"({total_kept / max(total_seen, 1) * 100:.2f}%) - Order Preserved"
    )
