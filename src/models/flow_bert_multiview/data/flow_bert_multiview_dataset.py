...
class MultiviewFlowDataModule(pl.LightningDataModule):
    ...
    def _create_datasets(self, stage_name):
        ...
        if split_mode == "session":
            ...
            # 根据UID划分flow数据集
            # 注意：这里为了保持 session 内部流的相对顺序，不应随意 shuffle，
            # 但 isin() 会打乱顺序，通常建议 merge 或者保持原始顺序的过滤
            # 下面这种写法会保留 flow_df 中的原始顺序（即时间顺序）
            train_flow_df = self.flow_df[self.flow_df['uid'].isin(set(train_flow_uids))]
            validate_flow_df = self.flow_df[self.flow_df['uid'].isin(set(validate_flow_uids))]
            test_flow_df = self.flow_df[self.flow_df['uid'].isin(set(test_flow_uids))]

        elif split_mode == "flow":

            # ✅ 获取随机配置：如果配置中没有 sampling.random，默认视为 True（保持原有随机行为）
            sampling_cfg = getattr(self.cfg.data, "sampling", None)
            is_random = True
            if sampling_cfg is not None:
                is_random = getattr(sampling_cfg, "random", True)

            logger.info(f"使用基于 flow_df 的逐流划分策略 | 随机打乱: {is_random}")
            ...
            if is_random:
                # =================================================
                # 模式 A: 随机打乱 + 分层采样 (Stratified Shuffle)
                # 适用：常规监督学习训练，防止某些类别在训练集中缺失
                # =================================================
                # logger.info("执行随机分层划分 (Stratified Shuffle Split)...")
                ...
                #     random_state=self.cfg.data.get("random_state", 42),
                #     shuffle=True  # 🔴 必须为 True 才能进行 stratify
                # )
                #
                # # 计算剩余部分在 temp_df 中的比例
                # # 原理：Val / (Val + Test)
                ...
                # =================================================
                # 模式 B: 严格时序切分 (Sequential Split)
                # 适用：概念漂移检测、零样本学习，严格保留时间顺序
                # =================================================
                # ✅ 必须确保这里是基于索引的切片，绝不能 shuffle
                logger.info("执行严格时序切片划分 (Sequential Split - No Shuffle)...")

                total_len = len(self.flow_df)
                train_end = int(total_len * self.flow_train_ratio)
                val_end = int(total_len * (self.flow_train_ratio + self.flow_validate_ratio))

                # 使用 iloc 保持物理顺序
                train_df = self.flow_df.iloc[:train_end]
                validate_df = self.flow_df.iloc[train_end:val_end]
                test_df = self.flow_df.iloc[val_end:]

                # 检查时间戳以确认（可选日志）
                if "conn.ts" in self.flow_df.columns:
                    logger.info(f"  Train: {train_df['conn.ts'].min()} -> {train_df['conn.ts'].max()}")
                    logger.info(f"  Test : {test_df['conn.ts'].min()} -> {test_df['conn.ts'].max()}")
        ...
        logger.info(
            f"数据集划分: 训练集 {len(train_flow_df)}, 验证集 {len(validate_flow_df)}, 测试集 {len(test_flow_df)}")
