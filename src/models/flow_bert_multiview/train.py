...
def main(cfg: DictConfig):
    ...
    try:
        ...
        if hasattr(model, 'drift_detector') and model.drift_detector is not None:
            logger.info("\n" + "=" * 60)
            logger.info("📊 训练期间概念漂移检测汇总报告")
            logger.info("=" * 60)

            stats = model.drift_detector.get_statistics()
            logger.info(f"  总处理样本数: {stats['total_samples']}")
            logger.info(f"  检测到漂移次数: {stats['drift_count']}")

            if stats['drift_count'] > 0:
                logger.warning(f"⚠️ 训练期间检测到{stats['drift_count']}次概念漂移")
                logger.info(f"  建议检查数据分布稳定性")
        ...
        best_model = FlowBertMultiview.load_from_checkpoint(best_model_path, cfg=cfg, dataset=datamodule.train_dataset)
