                    ...
                    def on_test_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx=0):
                        ...
                        if self.batch_count % self.check_interval == 0:
                            if hasattr(pl_module, 'drift_detector') and pl_module.drift_detector is not None:
                                # 这里只负责记录日志，具体的检测逻辑已经在 model.test_step 中执行了
                                result = pl_module.drift_detector.detect_drift()
                                # 注意：result 是 (is_drift, B, info)
                                if result:
                                    is_drift, B, info = result
                                    if is_drift:
                                        logger.warning(f"🚨 [TEST阶段] Batch {batch_idx} 检测到漂移! Bayes Factor: {B:.4f}")
