...
class DriftHandler:
    """
    处理概念漂移检测与适应的中间层
    封装了 Detector 和 Adapter，并适配 train.py 的接口需求
    """

    def __init__(self, model, cfg: DictConfig):
        self.model = model
        self.cfg = cfg
        self.enabled = cfg.training.drift_monitor.enabled
        self.detector = None
        self.adapter = None
        self.last_drift_result = (False, 0.0, {})  # (is_drift, statistic, info)

        # 记录漂移次数
        self.drift_count = 0

        if self.enabled and BNDMDetector is not None:
            self._init_components()

    def _init_components(self):
        # 1. 初始化检测器
        algo = self.cfg.get('concept_drift', {}).get('algorithm', 'bndm')
        det_params = {
            'seed': self.cfg.get('data', {}).get('random_state', 2026),
            'threshold': 0.05,
            'window_size': 1000
        }
        # 合并配置
        if 'detectors' in self.cfg.get('concept_drift', {}) and algo in self.cfg.concept_drift.detectors:
            det_params.update(self.cfg.concept_drift.detectors[algo])

        logger.info(f"🛡️ 初始化漂移检测器: {algo.upper()}")
        if algo == 'bndm':
        ...
        # 2. 初始化适配器 (仅在 IncrementalAdapter 可用时)
        if IncrementalAdapter is not None:
            adapter_config = self.cfg.get('concept_drift', {}).get('adaptation', {})
            # 确定任务模式 (二分类或多分类)
            ...
            logger.info(f"🔄 初始化增量适配器 (Mode: {task_mode})")

    def update_batch(self, features: torch.Tensor, labels: torch.Tensor, adapt: bool = False):
        """
        处理一个 batch 的数据：
        1. 预处理并更新检测器
        2. 如果检测到漂移且 adapt=True，触发适配器
        """
        ...
        # 逐样本更新检测器 (因为 BNDM/ADWIN 通常是基于流的)
        for i in range(features.size(0)):
            # 1. 降维预处理 (Vector -> Scalar)
            val = self.detector.preprocess(features[i])
        ...
        if batch_drift_detected:
            self.drift_count += 1

        # 获取当前的统计量 (BNDM specific)
        if hasattr(self.detector, '_get_total_bf'):
            current_b_val = self.detector._get_total_bf()

        # 更新状态供 train.py 回调使用
        info = {
            'bayes_factor': current_b_val,
            'comparison': 'drifting' if batch_drift_detected else 'stable'
        }
        self.last_drift_result = (batch_drift_detected, current_b_val, info)

        # 3. 触发适应 (仅在测试/部署阶段)
        if batch_drift_detected and adapt and self.adapter is not None:
            logger.info("🚨 触发概念漂移适应 (Adaptation Triggered)")
            self.adapter.adapt(features, labels)

        return batch_drift_detected

    def detect_drift(self):
        """
        供 train.py 的 DriftMonitorCallback 调用
        返回上一次 batch 处理的检测结果
        """
    ...
    def get_statistics(self):
        ...
        if self.detector:
            ...
            if hasattr(self.detector, 'get_statistics'):
                ...
                # 读取 BNDM 特定属性
                if hasattr(self.detector, 'ref_window'):
                    stats['window_R_size'] = len(self.detector.ref_window)
                if hasattr(self.detector, 'cur_window'):
                    stats['window_W_size'] = len(self.detector.cur_window)
...
class FlowBertMultiview(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # 🟢 在训练步中更新检测器状态 (但不适应)
        # 这允许 train.py 的 DriftMonitorCallback 通过 self.drift_detector.detect_drift() 获取状态
        if self.drift_detector.enabled:
            # 仅提取 embedding 用于检测，不触发 adapt
            features = outputs['multiview_embeddings'].detach()
            labels = batch['is_malicious_label']
            self.drift_detector.update_batch(features, labels, adapt=False)
    ...
    def on_test_epoch_start(self):
        ...
        if self.drift_detector:
            ...
            logger.info("🔧 [Test Phase] 漂移检测器已重置，准备进行在线检测与适应")
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.drift_detector.enabled:
            ...
            # 这里 adapt=True，表示如果在测试集发现漂移，将触发 Adapter 进行微调
            is_drift = self.drift_detector.update_batch(features, labels, adapt=True)

            if is_drift:
                self.test_drift_steps.append(batch_idx)
                logger.info(f"🚨 Test Step {batch_idx}: Concept Drift Detected!")
    ...
    def _compute_and_log_is_malicious_epoch_metrics(
        ...
        if stage in ["test", "val"] and self.trainer.is_global_zero:
            logger.info("=" * 60)
