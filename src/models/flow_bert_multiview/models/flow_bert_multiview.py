...
class FlowBertMultiview(pl.LightningModule):
    ...
    @torch.no_grad()
    def extract_fusion_features(self, batch: Dict[str, Any]) -> torch.Tensor:
        """
        【新增接口】: 离线提取图节点表征。
        利用现有的 forward 逻辑安全地穿过所有 Encoder 和融合层，直接返回融合向量。
        """
        self.eval()
        outputs = self(batch)
        return outputs['multiview_embeddings']  # 返回 [batch_size, 768] 的张量
    ...
    def _compute_and_log_is_malicious_epoch_metrics(
        ...
        if stage in ["val","test"] and self.trainer.is_global_zero:
            logger.info("=" * 60)
