...
try:
    ...
    try:
        ...
        # 如果都失败，暂时设为 None (防止 import 报错阻断训练)
        BNDMDetector = None
        ADWINDetector = None
...
class FlowBertMultiview(pl.LightningModule):
    ...
    def __init__(self, cfg: DictConfig, dataset):
        ...
        drift_cfg = cfg.get('training', {}).get('drift_monitor', {})
        if drift_cfg.get('enabled', False) and BNDMDetector is not None:
            algo = cfg.get('concept_drift', {}).get('algorithm', 'bndm')
            det_params = {
                'seed': cfg.get('data', {}).get('random_state', 2026),
                'threshold': 0.01,
                'window_size': 1000
            }
            cd_cfg = cfg.get('concept_drift', {})
            if 'detectors' in cd_cfg and algo in cd_cfg['detectors']:
                det_params.update(cd_cfg['detectors'][algo])

            logger.info(f"🛡️ 初始化漂移检测器: {algo.upper()}")
            if algo == 'bndm':
                self.drift_detector = BNDMDetector(det_params)
            elif algo == 'adwin':
                self.drift_detector = ADWINDetector(det_params)
