...
class DriftHandler:
    ...
    def update_batch(self, features: torch.Tensor, labels: torch.Tensor, adapt: bool = False):
        """
        处理一个 batch 的数据 (严格对齐 run_experiment.py 的逻辑)
        """
        ...
        for i in range(features.size(0)):
            ...
            if is_drift:
                ...
                # 4. 🚨 关键修复: 发现漂移 -> 立即重置！
                # run_experiment.py 中每次 update 发现漂移都会 reset。
                # 如果不 reset，检测器状态会异常，后续样本无法正常检测，导致检测次数为 0。
                self.detector.reset()

                # 日志记录 (避免过多刷屏，可设阈值)
                if adapt:
                    logger.info(f"🚨 [Sample {i}] Detected Drift (B={current_b:.5f}), Detector Reset.")

        # Batch 结束后的汇总适应
        # 虽然我们在 loop 内 reset 了检测器，但为了效率，我们在 batch 结束后统一进行一次模型适应
...
class FlowBertMultiview(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.drift_detector.enabled:
            ...
            # 🚑 [修复] 使用 attack_family_label 作为多分类适应的标签 (如果可用)
            # 因为 IncrementalAdapter 在 multiclass 模式下需要具体的类别标签
            if "attack_family_label" in batch and self.attack_family_classifier is not None:
