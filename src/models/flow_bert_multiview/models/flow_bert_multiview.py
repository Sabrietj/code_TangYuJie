...
class FlowBertMultiview(pl.LightningModule):
    ...
    def on_train_start(self) -> None:
        ...
        # 初始化漂移管理器（注入 self 以便访问 trainer 和 optimizers）
        if self.drift_enabled and self.drift_manager is None:
            logger.info("🔧 初始化概念漂移管理器...")
            try:
                self.drift_manager = ConceptDriftManager(self.cfg, self)
                logger.info(f"✅ 主检测器: {self.drift_manager.main_name.upper()}")
                if self.drift_manager.shadow_detectors:
                    logger.info(f"🕵️ 影子检测器 (对比模式): {list(self.drift_manager.shadow_detectors.keys())}")
            except Exception as e:
    ...
    def forward(self, batch: Dict[str, Any]) -> Dict[str, torch.Tensor]:
        ...
        return {
            ...
            # 'multiview_embeddings': multiview_outputs,  # 重复键，可以删除这行
            # 'classification_logits': classification_logits, # 🔴 删除这一行 (它是导致 NameError 的原因)
        }

        # 2. 概念漂移检测逻辑 (通过 Manager 统一处理)
        if self.training and self.drift_enabled and self.drift_manager:
            try:
                # 降低检测频率，例如每 50 个 step 检测一次，减少计算开销
                if self.global_step % 50 == 0:
                    # process_batch 会处理检测、对比、以及触发适应策略(如LR Decay)
                    result = self.drift_manager.process_batch(
                        features=multiview_outputs.detach(),
                        global_step=self.global_step,
                        current_epoch=self.current_epoch
                    )

                    # 记录日志
                    if result['main_drift']:
                        self.log('drift/triggered', 1.0, on_step=True)
                        # 记录一些特定的统计信息
                        info = result.get('info', {})
                        if 'bayes_factor' in info:
                            self.log('drift/bayes_factor', info['bayes_factor'], on_step=True)
                    else:
                        self.log('drift/triggered', 0.0, on_step=True)

            except Exception as e:
                if self.debug_mode: logger.warning(f"Drift detection error: {e}")
    ...
    def test_step(self, batch, batch_idx):
        ...
        # 调用 _shared_step 返回 outputs，不重复计算 loss
        self._shared_step(batch, batch_idx, stage="test", return_outputs=False)

        # 4. 组件数据收集 added by qinyf 2025-12-02
        if self.cfg.shap.enable_shap:
