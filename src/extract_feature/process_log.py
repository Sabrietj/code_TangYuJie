...
from session_tuple import SessionTuple
...
class ProcessLogs(EvaluateData):
    ...
    def evaluate_features(self, path_to_dataset):
        self.conn_log = os.path.join(path_to_dataset, 'conn_label.log')
        self.ssl_log = os.path.join(path_to_dataset, 'ssl.log')
        self.x509_log = os.path.join(path_to_dataset, 'x509.log')
        self.dns_log = os.path.join(path_to_dataset, 'dns.log')
        self.flowmeter_log = os.path.join(path_to_dataset, 'flowmeter.log')
        ...
        self.create_session_tuple(path_to_dataset)
    ...
    # create conn-session_tuple from ssl.log
    def create_session_tuple(self, path_to_dataset):
        print("\n>>> creating session_tuple from " + self.ssl_log)
        background_flows = 0
        ...
        with open(self.ssl_log) as ssl_file:
            for ssl_line in ssl_file:
                ...
                # add ssl log to session_tuple
                if conn_index in self.session_tuple:
                    self.session_tuple[conn_index].add_ssl_log(ssl_line)
                else:
                    self.session_tuple[conn_index] = SessionTuple(conn_index)
                    self.session_tuple[conn_index].add_ssl_log(ssl_line)

                number_of_added_ssl += 1
                # add x509 log to session_tuple
                ...
                if '-' != cert_chain_fuids:
                    ...
                    if x509_uid in self.x509_dict:
                        ...
                        self.session_tuple[conn_index].add_x509_log(x509_line)
                        self.session_tuple[conn_index].is_SNI_in_cert(
                            ssl_line, x509_line)
    ...
    def load_x509_file(self):
        ...
        print("\n>>> load %s" % self.x509_log)


    '''
    Methods for adding not ssl flow from conn.log to connection-session_tuple
    '''
    def add_conn_log(self):
        ...
        with open(self.conn_log) as f:
            for conn_line in f:
                ...
                if conn_index in self.session_tuple:
                    if conn_uid in self.session_tuple[conn_index].get_ssl_uid():
                        self.session_tuple[conn_index].add_ssl_flow(
                            conn_line, label)
                        ssl_flow += 1
                    else:
                        self.session_tuple[conn_index].add_not_ssl_flow(
                            conn_line, label)
                        not_ssl_flow += 1
                else:
                    pass
    ...
    def print_ver_cipher_dict(self):
        ...
        for key in self.session_tuple:
            conn_logs += self.session_tuple[key].number_of_flows()
            ssl_logs += self.session_tuple[key].get_number_of_ssl_logs()
            for a in self.session_tuple[key].get_ssl_cipher_dict():
                try:
                    cipher_suite_dict[a] += self.session_tuple[
    ...
    def print_state_dict(self):
        ...
        for key in self.session_tuple:
            conn_logs += self.session_tuple[key].number_of_flows()
            ssl_logs += self.session_tuple[key].get_number_of_ssl_logs()
            for a in self.session_tuple[key].get_conn_state_dict():
                try:
                    temp_dict[a] += self.session_tuple[key].get_conn_state_dict(
                    )[a]
                except:
                    temp_dict[a] = self.session_tuple[key].get_conn_state_dict(
                    )[a]
    ...
    def print_cert_key_length_dict(self):
        ...
        x509_logs = 0
        temp_dict = dict()
        for key in self.session_tuple.keys():
            conn_logs += self.session_tuple[key].number_of_flows()
            ssl_logs += self.session_tuple[key].get_number_of_ssl_logs()
            x509_logs += self.session_tuple[key].get_size_of_x509_list()
            for a in self.session_tuple[key].get_cert_key_length_dict().keys():
                try:
                    temp_dict[a] += self.session_tuple[
    ...
    def print_version_of_ssl_dict(self):
        ...
        for key in self.session_tuple:
            conn_logs += self.session_tuple[key].number_of_flows()
            ssl_logs += self.session_tuple[key].get_number_of_ssl_logs()
            for a in self.session_tuple[key].get_ssl_version_dict():
                try:
                    temp_dict[a] += self.session_tuple[key].get_ssl_version_dict(
                    )[a]
                except:
                    temp_dict[a] = self.session_tuple[key].get_ssl_version_dict(
                    )[a]
